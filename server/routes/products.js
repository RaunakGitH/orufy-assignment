...
router.get('/', async (req,res)=>{
    try{
        ...
        const products = (await Product.find(filter)).toSorted({
            createdAt:-1
        })
        res.json({
            success:true,
...
router.post('/', async (req,res)=>{
    try{
        const {name , type , stock , mrp , sellingPrice , brandName , images , exchangeOrReturn } = req.body;
        ...
        const product = await Product.create({
            name , type , stock , mrp , sellingPrice , brandName , images , exchangeOrReturn
        })
        res.status(201).json({
            success:true,
